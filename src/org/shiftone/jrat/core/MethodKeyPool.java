...
/**
 * Class MethodKeyPool pools methodKey instances, and also assignes each a uniqie ID string of the form m0, m1, m2, m3, m4.  The
 * id's are useful for providers that would need to represent a single method multiple times in their output.  Instead of
 * duplicating the className, methodName, and signature in their output, the output can contain a method pool section, and
 * methods can be referenced by id elsewhere.
 *
...
public class MethodKeyPool {
    ...
    private static final Log LOG          = LogFactory.getLogger(MethodKeyPool.class);
    ...
    public synchronized MethodKey getMethodKey(String className, String methodName, String signature) {
        ...
        if (method2IdMap.containsKey(key)) {
            ...
            // it the method key does exist, DO NOT RETURN the newly created key.
            // instead, return the key contained within the map.  This keeps only
            // a single instance 'in play' of any one key, making the new one (above)
    ...
    /**
     * Method setMapping adds a NEW mapping.  It is assumed that the provided methodKey and id are not yet being stored in the
     * map.
     *
     * @param methodKey .
     * @param id .
    ...
    public synchronized void addMapping(MethodKey methodKey, String id) {
        ...
        // santity check.. these should never happen if the pool is used correctly.
